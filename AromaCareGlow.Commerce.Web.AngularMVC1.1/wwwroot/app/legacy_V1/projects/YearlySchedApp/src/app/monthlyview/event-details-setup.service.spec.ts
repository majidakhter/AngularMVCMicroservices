...
describe('EventDetailsSetupService', () => {
  ...
  const activityEvent: IEventData = {
    isTradeable: true,
  ...
  const paycodeEventNoConfig: IEventData = {
    isTradeable: true,
    eventType: EventTypes.PAYCODE,
    schedule: {
      payCode: {
  ...
  describe('#mapEvent', () => {
    describe('when mapping an activity', () => {
      ...
      it('should map the configuration flags', () => {
        expect(mappedEvent.activity.configuration.isDisplayedOnMonthlyView).toEqual(activityEvent.schedule.activity.configuration.isDisplayedOnMonthlyView);
        expect(mappedEvent.activity.configuration.isOnCall).toEqual(activityEvent.schedule.activity.configuration.isOnCall);
        expect(mappedEvent.activity.configuration.isTimeOff).toEqual(activityEvent.schedule.activity.configuration.isTimeOff);
      });
    ...
    describe('When mapping a paycode', () => {
      ...
      it('should map the configuration flags', () => {
        expect(mappedEvent.paycode.configuration.isOnCall).toEqual(paycodeEvent.schedule.payCode.configuration.isOnCall);
        expect(mappedEvent.paycode.configuration.isTimeOff).toEqual(paycodeEvent.schedule.payCode.configuration.isTimeOff);
      });
